class ContactsController < InheritedResources::Base
	...
	# 		Contact.find_or_create_by(company_name: find_company_name,email: technician_user.email,user_id: technician_user.id,is_completed: true,task_id: session[:task_id],role_id: technician_role.id)
	# 	rescue
	# 		return nil
	# 	else
	# 	end
	...
	def save_contact_main_contact
		begin
			...
			if contact.present?
				contact.update_attributes(company_name: find_company_name,email: main_contact_user.email,user_id: main_contact_user.id,is_completed: true,task_id: session[:task_id],role_id: main_contact_role.id)
			else
				Contact.create(company_name: find_company_name,email: main_contact_user.email,user_id: main_contact_user.id,is_completed: true,task_id: session[:task_id],role_id: main_contact_role.id)
			end
		rescue
			 return nil
		end
	...
	def find_company_name
		@name = Company.find_by(task_id: session[:task_id])&.company_name
	end
