module QuestionaireAnswersHelper
  ...
  def plan_edit
    @questionaire_answers.where('task_id = ? AND question_type_id = ?',session[:task_id], 1).order(:id).first.id
  end

  def fifty_five_hundred_edit
    @questionaire_answers.where('task_id = ? AND question_type_id = ?',session[:task_id], 2).order(:id).first.id
  end
